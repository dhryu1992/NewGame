...
void SaberBattle() {
    ...
    switch(SelectMotion) {
        HitAfterRun:
        case HIT : {
        while(saber->m_Hp > 0 && FirstMon->m_Hp > 0) {
            if(saber->m_Hp > 0) {
                cout << "\nNow, Your turn" << endl;
                saber->Hit(saber);
                FirstMon->injured(FirstMon, 5);
                cout << "saber Hp: " << saber->m_Hp << endl;
        ...
        switch(SelectMotion2) {
            case HIT : {
            HitAfterRunSec:
            while(saber->m_Hp > 0 && SecMon->m_Hp > 0) {
                if(saber->m_Mp < 50) {
                    cout << "You should dring Mp Potion" << endl;
                    mp_potion->Use(saber, 100, 5); 
